...
namespace Noga\Core\ReadFileng;

use JsonException;
use Noga\Core\CacheManager;
use Noga\Core\ReadFileng\Parse\Parse;
...
final class Read
{
    ...
    private function load(string $file): array
    {
        ...
        $this->matches = Parse::getConfig($file);
        $config = [];
 ...
 */
    ...
    private function ch():CacheManager{
        return CacheManager::key("readFilesng");
    }

    private function cache(array $data):void{
        if(!$this->ch()->hasValidSignature($data)){
        CacheManager::key("readFilesng")
            ->signature($data)
            ->data($data)
            ->put();
        }
