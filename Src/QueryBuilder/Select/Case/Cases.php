...
class Cases
{
    ...
    public function when(string $condition, string $then, ?string $bindValue = ""): static
    {
        if (str_contains($condition, '?')) {
            throw new InvalidArgumentException("Bind '?' cannot used : use :name");
        }
    ...
    {
        $paramElse = BindHashing::hash("else", $value) . (++$this->counter);
        $this->else = $paramElse;
        $this->params[$paramElse] = $value;
        return $this;
    }
     ...
     * @return static
     */
    public function as(?string $alias = null): static
    {
        $this->alias = ($alias !== null) ? $alias : "case_" . $this->counter++ . "";
        return $this;
    }
     ...
     * @return static
     */
    public function selectCaseFormArray(array $array): static
    {
        foreach ($array as $block) {
            ...
            foreach ($block['cases'] as $case) {
                ...
                $name = $this->matchCond($cond);
                $this->params[":$name"] = $value;
            }

            $this->caseFromArray[] = " CASE {$column} " . implode(" \n", $whens) . " ELSE '$else' END AS $alias ";
        }
